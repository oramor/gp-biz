CREATE OR REPLACE PROCEDURE adm.create_db_table_for_dic(OUT p_db_table_id integer, IN p_entity_id integer)
 LANGUAGE plpgsql
AS $procedure$
...
BEGIN
	...
	WHERE id = p_entity_id AND t.is_document = false;
	IF NOT FOUND THEN
		RAISE EXCEPTION 'Not found entity with id % and doc false', p_entity_id;
	END IF;
	...
	FOR r_column IN SELECT * FROM abstract_column t WHERE t.is_for_dic = TRUE
	ORDER BY t.priority
	LOOP 
